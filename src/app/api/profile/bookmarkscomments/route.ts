...
export async function DELETE(req: NextRequest) {
  ...
  for (const comment of commentsToDelete) {
    ...
    // 댓글 삭제
    if (category === 'archive') {
      const { error } = await supabase.from('archive_comment_bookmarks').delete().eq('comment_id', id);
      if (error) throw new Error(`댓글 삭제 실패 (archive): ${error.message}`);
    } else if (category === 'forum') {
      const { error } = await supabase.from('forum_comment_bookmarks').delete().eq('comment_id', id);
      if (error) throw new Error(`댓글 삭제 실패 (forum): ${error.message}`);
    } else if (category === 'qna') {
      const { error } = await supabase.from('qna_comment_bookmarks').delete().eq('comment_id', id);
